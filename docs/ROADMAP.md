...
- зафиксировать предварительную модель БД;
- определить API-контракт;
- описать основные UX-сценарии Telegram и Web;
- зафиксировать роли и права;
...
- ✅ правила разработки;
- ⏳ финальная сверка документов между собой;
- ⏳ первоначальный backlog для Этапа 1 перед началом кодинга.

### Критерий завершения Этапа 0

Перед началом разработки должны быть согласованы как минимум:
...
После этого можно создавать кодовый scaffold и переходить к реализации первой вертикальной возможности.
...
### Предпочтительный порядок реализации внутри Этапа 1

Не создавать сразу весь проект целиком. Идти вертикальными срезами:

```text
1. Identity + доступ
2. DNS zones read model
3. Создание поддомена
4. Просмотр доменов
5. DNS records CRUD
6. Удаление домена
7. Audit
8. Web и Telegram доводятся до одинакового функционального покрытия
```

Каждый срез должен проходить через одно application core и быть доступен обоим клиентам без дублирования бизнес-логики.
